CREATE PROCEDURE
	...
	[dbo].[Herding_Csad] @CreditRisk NVARCHAR(10) = NULL
...
BEGIN
            ...
            FROM (
                ...
                INNER JOIN (
                    ...
                    WHERE
						...
						AND RatingNum > CASE WHEN NULL = 'HY' THEN 10 ELSE 0 END
						AND RatingNum < CASE WHEN NULL = 'IG' THEN 11 ELSE 25 END
    ...
    SELECT
        *,
        CASE
            ...
            WHEN LagWeightedPrice = 0 THEN NULL
            ELSE ( 
                WeightPrice + 
    ...
    FROM (
        SELECT
            ...
            Rm,
            ABS(SUM(R) - Rm) AS Sum,
            COUNT(DISTINCT CusipId) AS Count
        FROM (
            SELECT
                CusipId,
                TrdExctnDtEOM,
                R,
                SUM(WeightedReturn) OVER (PARTITION BY TrdExctnDtEOM) / SUM(Volume) OVER (PARTITION BY TrdExctnDtEOM) AS Rm
            FROM (
                SELECT
                    *,
                    Volume * R AS WeightedReturn
                FROM
                    #TEMP_FINAL
                WHERE
                    R IS NOT NULL
