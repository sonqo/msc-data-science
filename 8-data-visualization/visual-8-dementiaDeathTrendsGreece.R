...
p10 <-
  ...
  geom_smooth(
    aes(x=year, y=dementia_deaths, color=geo_code), size=0.75, method = "loess", se = FALSE
  ) +
  theme_bw() +
  scale_color_manual(values=c("blue","gold")) +
  scale_x_continuous(breaks = 2011:2021, labels = unique(dementia_trend$year)) +
  labs(title = "Absolute Dementia Deaths Trend | Greece vs Europe\n", x = '', y = 'Dementia Deaths', color='Region') +
  theme(plot.title = element_text(hjust = 0.5))

p11 <- 
  ggplot(dementia_trend) +
  geom_smooth(
    aes(x=year, y=dementia_death_ratio, color=geo_code), size=0.75, method = "loess", se = FALSE
  ) +
  theme_bw() +
  scale_color_manual(values=c("blue","gold")) +
  scale_x_continuous(breaks = 2011:2021, labels = unique(dementia_trend$year)) +
  labs(title = "Dementia Death Ratio Trend | Greece vs Europe\n", x = '', y = 'Dementia Death Ratio [%]', color='Region') +
  theme(plot.title = element_text(hjust = 0.5))
